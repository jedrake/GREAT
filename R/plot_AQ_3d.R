...
aq.m <- summaryBy(Photo+Tleaf+PARi~TleafFac+LightFac+prov+location,data=subset(aq,campaign==1 & W_treatment=="w"),FUN=c(mean,standard.error))
...
aq.l <- split(aq.m,paste(aq.m$location,aq.m$TleafFac,sep="-"))
...
aq.l2 <- split(aq.m,paste(aq.m$location,aq.m$LightFac,sep="-"))
